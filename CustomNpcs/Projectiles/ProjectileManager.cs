...
namespace CustomNpcs.Projectiles
{
	public class ProjectileManager : CustomTypeManager<ProjectileDefinition>, IDisposable
	{
		...
		public static void SendProjectileUpdate(int index) => TSPlayer.All.SendData(PacketTypes.ProjectileNew, "", index);//Debug.Print($"Sent projectile new or update for index #{index}!");

		public static void SendProjectileKill(int index, int owner = 255) => TSPlayer.All.SendData(PacketTypes.ProjectileDestroy, "", index, owner);//Debug.Print($"Sent projectile destroy for index #{index}!");
